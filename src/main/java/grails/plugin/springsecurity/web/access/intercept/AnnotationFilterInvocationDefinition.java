...
public class AnnotationFilterInvocationDefinition extends AbstractFilterInvocationDefinition implements ServletContextAware {
    ...
    protected String determineUrl(final FilterInvocation filterInvocation) {
        ...
        try {
            ...
            if (grails23Plus) {
                urlInfos = grails.plugin.springsecurity.ReflectionUtils.matchAllUrlMappings(urlMappingsHolder, requestUrl, grailsRequest, mimeTypeResolver);
            } else {
    ...
    /**
     * Dependency injection for the mimeTypeResolver bean.
     *
     * @param api the bean
     */
    public void setMimeTypeResolver(MimeTypeResolver api) {
        mimeTypeResolver = api;
    }
